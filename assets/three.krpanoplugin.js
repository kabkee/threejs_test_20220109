...
function krpanoplugin()
{
	...
	local.registerplugin = function(krpanointerface, pluginpath, pluginobject)
	{
        ...
        // load the requiered three.js scripts
        load_scripts(["three.min.r126.js", "GLTFLoader.js"], start);
	...
	function load_object_gl(url, animated, properties, donecall)
	{
		orgUrl = url;
	...
	function build_scene()
	{
		...
		// load 3d objects
        load_object_gl("models/free_porsche_911_carrera_4s/scene.gltf", false, {
			ath: -30,
			atv: 22,
			depth: 1000,
			scale: 50.0,
		...
		// boxTexture.load(resolve_url_path("box.jpg"), function(texture){
		// 	box = new THREE.Mesh(new THREE.BoxGeometry(500,500,500), new THREE.MeshBasicMaterial({map: texture}));
        ...
        raycasterHelper = new THREE.ArrowHelper(camera_hittest_raycaster.ray.direction, camera_hittest_raycaster.ray.origin, 100, 0x00ff00);
	...
	function do_object_hittest(mx, my)
	{
        ...
        raycasterHelper.setDirection(camera_hittest_raycaster.ray.direction);
	...
	function update_scene()
	{
		...
		{
			// animatedobjects[i].update(1000 * delta);
			animatedobjects[i].update( 1000* delta );
		}
